...
export class DependencyGraph {
    ...
    async findPackages() {
        ...
        const { packages } = await getPackages(path).catch(() => {
            this.#logger.error('Could not any package', '❌', 'red');
    ...
    async generate() {
        const excluded = this.#options.getExcluded();
    ...
    printOrder() {
        this.#order.forEach(pkg => this.#logger.log(this.#logger.format(pkg.name, 'green'), '‣'));
    }
    ...
    printGraph() {
        ...
        entries.forEach(([name, dependencies]) => {
            ...
            const formattedVersion = pkg ? this.#logger.format(`@ v${pkg.version}`, 'gray') : '';
            this.#logger.log(`${formattedName} ${formattedVersion}`, '‣');
    
            dependencies.forEach(dependency => this.#logger.log(this.#logger.format(dependency, 'gray'), '   '));
        });
    }
