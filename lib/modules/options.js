...
export class CliOptions {
  ...
  #loadFromCli(options = {}) {
    if (options.only)
      this.#options.only.push(...String(options.only || "").split(","));
    if (options.exclude)
      this.#options.exclude.push(...String(options.exclude || "").split(","));
    if (options.listeners)
      this.#options.listeners.push(
        ...String(options.listeners || "").split(",")
      );
    if (options.customLoggers)
      this.#options.customLoggers = options.customLoggers;
