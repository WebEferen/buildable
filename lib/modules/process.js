...
export class ProcessManager {
  ...
  setExecutions(queue = [], dependencies = []) {
    ...
    if (only.length > 0) {
      executions = this.#parser.parseOnly(executions, only, dependencies);
    }

    this.#logger.printPackages(executions.length);
    this.#parser.parseExecutions(executions, dependencies, (execution) => {
  ...
  spawn() {
    ...
    this.#queue.subscribe(async (execution) => {
      this.#execute([execution]).catch((error) => {
        ...
        this.#logger.error(message, `[${execution.name}]`, "red");
  ...
  #process(command) {
    ...
    this.#listener.detachCrashListener(command);
    this.#listener.attachCrashListener(command, this.#queue);

    this.#listener.detachInfoListener(command);
  ...
  #isServiceReady(output) {
    ...
    if (isMatch && this.#executions.length === 0) {
      ...
      this.#logger.printReadiness(processes);
      this.#logger.listenForKeyPress((key) => {
        if (key.name === "q") return process.exit(0);

        processes.forEach((proc, idx) => {
          if (idx > 8) return; // Only numeric keyboard allowed
          if (key && key.name === (idx + 1).toString())
            return this.#restartService(proc);
        });
      });
    }
