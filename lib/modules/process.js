...
export class ProcessManager {
    ...
    #isServiceReady(output) {
        // TODO: Add better watching checker (for example add this to the configuration options as the custom regex)
        if (output.includes('Watching for file changes') && this.#executions.length > 0) {
            this.#queue.next(this.#executions.shift());
            return true;
        }
