...
export class ProcessManager {
  ...
  setExecutions(queue = [], dependencies = {}) {
    ...
    if (only.length > 0) {
      // Check whether only has dependencies -> if yes, then don't remove them from executions list
      only.forEach((entry) => {
      ...
      executions = executions.filter((entrypoint) =>
        only.includes(entrypoint.name)
      );
    }
