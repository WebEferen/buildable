...
export class ProcessManager {
  ...
  spawn() {
    this.#queue.on("next", async (execution) => {
      if (!execution) return;
    ...
    this.#queue.emit("next", this.#executions.shift());
  }
  ...
  #process(command) {
    ...
    this.#listener.attachInfoListener(command, (output) => {
      ...
      if (this.#isServiceReady(output, this.#executions)) {
        return this.#queue.emit("next", this.#executions.shift());
      }
    });

    this.#processes[command.name].process.on("exit", (code) => {
      const emit = this.#queue.emit;

      switch (code) {
        case null:
          return emit("next", this.#initial[command.name]);
        case 0:
          return this.#executions.length > 0
            ? emit("next", this.#executions.shift())
            : null;
        default:
          return emit("next", { name: command.name });
      }
    });
  }
  ...
  #killServices() {
    Object.keys(this.#processes).forEach((service) => {
      ...
      return kill(this.#processes[service].process.pid);
    });
